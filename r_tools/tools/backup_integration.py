# ./tools/r_tools/tools/backup_integration.py
...
def get_backup_info() -> dict[str, Any]:
    ...
    return {
        ...
        "profiles_default": (_load_json(profiles).get("default") if profiles.is_file() else None),
        "profiles_names": (sorted((_load_json(profiles).get("profiles") or {}).keys()) if profiles.is_file() else []),
    }
...
def _build_backup_cmd(overrides: dict[str, Any]) -> tuple[list[str], Path]:
    ...
    if not prof.is_file():
        raise FileNotFoundError(f"backup: Fant ikke {prof}\n" f"Tips: opprett profiler i {prof}")
